        ...
        <div class="card">
          ...
          <div class="card-body">
            <form action="{{ route('posts.update' , $post->id) }}" method="POST" autocomplete="on"
              ...
              <div class="form-group row mb-4">
                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{__('Keywords')}}</label>
                <div class="col-sm-12 col-md-7">
                  <input type="text" data-role="tagsinput" class="form-control  @error('keywords') is-invalid @enderror" name="keywords"
                    value="{{ $post->keywords }}" required placeholder="Keywords">
                  @error('keywords')
                  <div class="invalid-feedback">
                    <strong>{{ $message }}</strong>
                  </div>
